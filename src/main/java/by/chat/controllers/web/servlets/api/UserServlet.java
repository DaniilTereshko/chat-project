...
public class UserServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if ((login == null) || (login.equals(""))) {
            result+="?" + ERROR + "=" + UserException.LOGIN_ERROR.ordinal();
            resp.sendRedirect(result);
        } else if(userService.get(login) != null){
            result+="?" + ERROR + "=" + UserException.USER_EXIST.ordinal();
            resp.sendRedirect(result);
        } else if((password == null) || (password.equals(""))){
            result+="?" + ERROR + "=" + UserException.PASSWORD_ERROR.ordinal();
            resp.sendRedirect(result);
        } else if (!password.equals(confirmedPassword)) {
            result+="?" + ERROR + "=" + UserException.PASSWORDS_MATCH_ERROR.ordinal();
            resp.sendRedirect(result);
        } else if ((firstName == null) || (firstName.equals(""))) {
            result+="?" + ERROR + "=" + UserException.FIRSTNAME_ERROR.ordinal();
            resp.sendRedirect(result);
        } else if ((middleName == null)||(middleName.equals(""))) {
            result+="?" + ERROR + "=" + UserException.MIDDLENAME_ERROR.ordinal();
            resp.sendRedirect(result);
        } else if ((lastName == null) || (lastName.equals(""))) {
            result+="?" + ERROR + "=" + UserException.LASTNAME_ERROR.ordinal();
            resp.sendRedirect(result);
        } else if ((birthday == null) || (birthday.equals("  -  -    ")) || (birthday.equals("")))  {
            result+="?" + ERROR + "=" + UserException.BIRTHDAY_ERROR.ordinal();
            resp.sendRedirect(result);
        } else {
            ...
            try {
                ...
                Map<UserException, String> invalidFields = e.getInvalidFields();
                session.setAttribute("errors", invalidFields);
                resp.sendRedirect(result);
                return;
            }
